...
class LossFunctions:
  ...
  def log_normal(self, x, mu, logvar):
    """Logarithm of normal distribution with mean=mu and variance=exp(logvar)

    Args:
        x: (torch.Tensor) corresponding tensor containing the input
        mu: (torch.Tensor) corresponding tensor containing the mean
        logvar: (torch.Tensor) corresponding tensor containing the log-variance

    Returns:
        output: (torch.Tensor) a tensor with the log-likelihood for each sample
    """
    # The log-variance is more numerically stable than variance
    var = torch.exp(logvar)

    # Calculate log-likelihood using the formula
    # log(p(x)) = -0.5 * (log(2*pi) + log(var) + (x - mu)^2 / var)
    log_prob = -0.5 * (torch.log(torch.tensor(2.0 * torch.pi)) + logvar + torch.pow(x - mu, 2) / var)
    
    # Sum over the dimensions of the latent variable
